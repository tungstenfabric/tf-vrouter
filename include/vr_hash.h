...
static inline uint32_t vr_hash(const void *key, uint32_t length, uint32_t initval)
{
    ...
    if (length == 12) c += (uint32_t)k[11]<<24;
    if (length >= 11) c += (uint32_t)k[10]<<16;
    if (length >= 10) c += (uint32_t)k[9]<<8;
    if (length >= 9) c += k[8];
    if (length >= 8) b += (uint32_t)k[7]<<24;
    if (length >= 7) b += (uint32_t)k[6]<<16;
    if (length >= 6) b += (uint32_t)k[5]<<8;
    if (length >= 5) b += k[4];
    if (length >= 4) a += (uint32_t)k[3]<<24;
    if (length >= 3) a += (uint32_t)k[2]<<16;
    if (length >= 2) a += (uint32_t)k[1]<<8;
    if (length >= 1) {
        a += k[0];
