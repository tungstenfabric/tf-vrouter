    ...
    struct rte_ether_addr *src_mac, struct rte_ether_addr *dst_mac)
{
    ...
    assert((raw_encap_action_conf.size + sizeof(*hdr)) <= ENCAP_DECAP_BUFFER_SIZE);
...
set_action_raw_encap_ipv4_hdr(rte_be32_t src_ip, rte_be32_t dst_ip)
{
    ...
    assert((raw_encap_action_conf.size + header_size) <= ENCAP_DECAP_BUFFER_SIZE);
    ...
    struct rte_ether_addr *src_mac, struct rte_ether_addr *dst_mac)
{
    ...
    assert((raw_decap_action_conf.size + sizeof(*hdr)) <= ENCAP_DECAP_BUFFER_SIZE);
...
set_action_raw_decap_ipv4_hdr(rte_be32_t src_ip, rte_be32_t dst_ip)
{
    ...
    const uint8_t version_ihl = RTE_IPV4_VHL_DEF;
    const size_t  header_size = (version_ihl & RTE_IPV4_HDR_IHL_MASK) * RTE_IPV4_IHL_MULTIPLIER;
    assert((raw_decap_action_conf.size + header_size) <= ENCAP_DECAP_BUFFER_SIZE);
...
set_action_nat(const struct vr_n3k_offload_entry *entry)
{
    if (entry->flow->flags & VR_FLOW_FLAG_SNAT) {
        set_action_set_src_ipv4(entry->reverse_flow->dst_ip);
    }

    if (entry->flow->flags & VR_FLOW_FLAG_DNAT) {
        set_action_set_dst_ipv4(entry->reverse_flow->src_ip);
    }
}
...
convert_encap(struct vr_n3k_offload_entry *entry)
{
    if (entry->tunnel_type == VR_N3K_OFFLOAD_TUNNEL_VXLAN) {
        ...
        memcpy(
            outer_eth_spec.src.addr_bytes, entry->dst_nh->src_mac, VR_ETHER_ALEN);

        memcpy(
            outer_eth_spec.dst.addr_bytes, entry->dst_nh->dst_mac, VR_ETHER_ALEN);

        outer_ipv4_spec.hdr.src_addr = entry->dst_nh->tunnel_src_ip;
        outer_ipv4_spec.hdr.dst_addr = entry->dst_nh->tunnel_dst_ip;
    ...
    if (entry->tunnel_type == VR_N3K_OFFLOAD_TUNNEL_MPLS) {
        set_action_raw_encap();

        set_action_raw_encap_ether_hdr((struct rte_ether_addr *)entry->dst_nh->src_mac,
            (struct rte_ether_addr *)entry->dst_nh->dst_mac);

        set_action_raw_encap_ipv4_hdr(
            entry->dst_nh->tunnel_src_ip,
            entry->dst_nh->tunnel_dst_ip);
...
set_action_mpls_over_udp_decap(struct vr_n3k_offload_entry *entry)
{
    ...
    set_action_raw_decap();
    set_action_raw_decap_ether_hdr(
        (struct rte_ether_addr *)entry->src_nh->dst_mac,
        (struct rte_ether_addr *)entry->src_nh->src_mac);
    ...
    struct vr_n3k_offload_entry *entry, struct rte_flow_action **out_actions)
{
    ...
    if (entry->dst_nh->type == NH_TUNNEL) {
        ...
        if (entry->src_nh->type == NH_TUNNEL) {
            ...
            if (entry->route_traffic)
                set_action_route(entry->dst_nh->src_mac, entry->dst_nh->dst_mac);
        }
    }

    set_action_port_id(entry->dst_vif->port_id);

    if (entry->mirror_vif) {
        const bool modified = entry->src_nh->type == NH_TUNNEL;
        set_action_mirror(entry->mirror_vif->port_id, modified);
    }
