...
struct vr_dpdk_n3k_service_core n3k_service_cores[] = {
    VR_DPDK_N3K_SERVICE_CORE_INITIALIZER(mp_service, NULL, NULL),
    VR_DPDK_N3K_SERVICE_CORE_INITIALIZER(dispatcher, NULL, NULL),
...
#define FOREACH_ENABLED_N3K_SERVICE_CORE(iter, ptr)                            \
    ...
    for (; (iter) < RTE_DIM(n3k_service_cores); ++(iter), ++(ptr))             \
        if (is_service_core_enabled((ptr)))
