...
/* Update device info */
static void
dpdk_ethdev_info_update(struct vr_dpdk_ethdev *ethdev, struct vr_interface *vif)
{
    struct rte_eth_dev_info dev_info;
    ...
    if (vif && (vif->vif_flags & VIF_FLAG_ALL_MULTICAST_ENABLE)) {
        ...
        unsigned type, uint8_t vif_idx)
{
    ...
    sprintf(if_name_buf,"%s %s %x", info->intf_name, info->intf_drv_name,
            vif_idx);
...
static int
vr_dpdk_bond_send_port_info(uint16_t port_id, uint8_t vif_idx)
{
    int ret;
    char *str[] = {"UP", "DOWN"};
...
static int
vr_dpdk_bond_intf_callback(uint16_t port_id, enum rte_eth_event_type type,
...
vr_dpdk_ethdev_init(struct vr_dpdk_ethdev *ethdev, struct rte_eth_conf *dev_conf)
{
    ...
    dpdk_ethdev_info_update(ethdev, vif);
