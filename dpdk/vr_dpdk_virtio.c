...
struct rte_port_in_ops vr_dpdk_virtio_reader_ops = {
    ...
    .f_stats = NULL
    // TODO dpdk_virtio_reader_stats_read
};
...
// TODO :IF_NAME_SZ, MAX_VHOST_DEVICE   defined in (vhost.h)
    /* init virtio queue */
...
dpdk_virtio_from_vm_rx(void *port, struct rte_mbuf **pkts, uint32_t max_pkts)
{
    ...
    nb_pkts = rte_vhost_dequeue_burst(vru_cl->vruc_vid, vring_idx,
                                      vr_dpdk.rss_mempool, pkts, (uint16_t)max_pkts);
