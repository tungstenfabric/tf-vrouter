...
dpdk_virtio_from_vm_rx(void *port, struct rte_mbuf **pkts, uint32_t max_pkts)
{
    ...
    nb_pkts = rte_vhost_dequeue_burst(vru_cl->vruc_vid, vring_idx,
...
#endif
static inline void
dpdk_virtio_send_burst(int vid, struct dpdk_virtio_writer *p)
{
    uint16_t nb_tx = 0;
    ...
    if (likely(p->tx_buf_count)) {
        ...
        nb_tx = rte_vhost_enqueue_burst(vid, vring_idx,
                                        p->tx_buf, p->tx_buf_count);
        if (!nb_tx) {
            // error
            RTE_LOG_DP(ERR, UVHOST, "%s:TX Unable to send Pkt"
                       "vid %d, queue %p\n", __func__, vid, vq);
        }
...
dpdk_virtio_to_vm_tx(void *port, struct rte_mbuf *pkt)
{
    ...
    if (unlikely(p->tx_mbufs >= VR_DPDK_VIRTIO_TX_BURST_SZ)) {
        dpdk_virtio_send_burst(vru_cl->vruc_vid, p);
        if (lcore) {
