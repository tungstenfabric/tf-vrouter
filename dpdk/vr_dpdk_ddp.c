...
vr_dpdk_ddp_add(uint16_t port_id)
{
    ...
    char ddp_fbkp[] = "/usr/bin/mplsogreudp.bkp";
    ...
    if(ret == -ENOTSUP) {
        ...
        if(ddp_bkp_fd == NULL) {
            close(ddp_fd);
            vr_free(buf, VR_INFO_REQ_OBJECT);
            RTE_LOG(ERR, VROUTER, "%s: Failed to open mplsogre.bkp\n",
                        __func__);
            return -1;
        }
        ...
        if(fwrite(buf, 1, st_buf.st_size, ddp_bkp_fd) != st_buf.st_size) {
            ...
            fclose(ddp_bkp_fd);
            vr_free(buf, VR_INFO_REQ_OBJECT);
            RTE_LOG(ERR, VROUTER, "%s: Failed to write mplsogre.bkp\n",
                        __func__);
            return -1;
        }
        fclose(ddp_bkp_fd);
...
vr_dpdk_ddp_del(uint16_t port_id)
{
    ...
    char ddp_fpath[] = "/usr/bin/mplsogreudp.bkp";
    ...
    ret = rte_pmd_i40e_process_ddp_package(1, buf, st_buf.st_size,
