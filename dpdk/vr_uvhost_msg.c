...
vr_uvh_new_device_cb(int vid)
{
    ...
    vr_uvhost_log("%s: Number of vrings %d in vhost device with id %d\n",
        __func__, rte_vhost_get_vring_num(vid), vid);

    /* Disable kick notifications */
    for (i = 0; i < rte_vhost_get_vring_num(vid); i++) {
        ret = rte_vhost_enable_guest_notification(vid, i, 0);
        if (ret != 0) {
            vr_uvhost_log("%s: Failed to disable kick on vring %d\n",
                __func__, i);
        }
    }
    return 0;
