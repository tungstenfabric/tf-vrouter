...
static void vr_uvh_destroy_device_cb(int vid);
                       ...
                       int enable)
{
    ...
    if (enable && vq->vdv_ready_state != VQ_READY) {
        /*
        ...
        if (vr_dpdk_set_virtq_ready(vru_cl->vruc_idx, vring_idx, VQ_NOT_READY)) {
            vr_uvhost_log("Client %s: error setting vring %u ready state\n",
                    uvhm_client_name(vru_cl), vring_idx);
            return -1;
        }

        vr_uvhost_log("Client %s: vring %d is not ready\n",
                uvhm_client_name(vru_cl), vring_idx);
        return 1;
    }
...
vr_uvhm_set_vring_enable(vr_uvh_client_t *vru_cl, uint16_t vring_idx, int enable)
{
    ...
    uvhm_check_vring_ready(vru_cl, vring_idx, enable);
...
vr_uvh_nl_vif_add_handler(vrnu_vif_add_t *msg)
{
    ...
    ret = rte_vhost_driver_callback_register(file_path,
                                             &dpdk_rte_vhost_device_ops);
    if (ret != 0) {
...
error:
    if (ret != 0 && vru_cl) {
        ...
        vr_uvh_nl_send_intf_state(0, vru_cl->vruc_idx,
                                  basename(vru_cl->vruc_path));
...
{
    vr_uvh_client_t *vru_cl;
    uint64_t support_flags, features, stored_features;
    uint8_t is_gso_vm = 1;
    ...
    vr_uvhost_log("    GET FEATURES: 0x%"PRIx64"\n",
                                      features);
     ...
     */
    support_flags = (1ULL << VIRTIO_NET_F_HOST_TSO4) |
                    (1ULL << VIRTIO_NET_F_HOST_TSO6) |
                    (1ULL << VIRTIO_NET_F_GUEST_TSO4) |
                    (1ULL << VIRTIO_NET_F_GUEST_TSO6) ;
    is_gso_vm = features & support_flags;
    if (vif) {
        if (!!is_gso_vm) {
            vif->vif_flags |= VIF_FLAG_GRO_NEEDED;
            vr_uvhost_log("%s: GRO needed for vid %d\n", __func__, vid);
        } else {
            vif->vif_flags &= ~VIF_FLAG_GRO_NEEDED;
        }
    }
    ...
    if (dpdk_check_rx_mrgbuf_disable() == 0)
        vr_dpdk_store_persist_feature(uvhm_client_name(vru_cl),
                                      features);
    vru_cl->vruc_flags |= VRUC_FLAG_SET_FEATURE_DONE;
    // VCSR-151: state of workload went up from down
    /* Send netlink interface up message to agent */
    vr_uvh_nl_send_intf_state(1, vru_cl->vruc_idx,
                              basename(vru_cl->vruc_path));
    return 0;
}
...
void vr_uvh_destroy_device_cb(int vid)
{
    ...
    for (i = 0; i < rte_vhost_get_vring_num(vid); i++) {
        vr_uvhm_set_vring_enable(vru_cl, i, 0);
        vr_uvhm_set_vring_enable(vru_cl, i, 0);
    }
}
...
vr_uvh_vring_state_changed_cb(int vid,  uint16_t vring_idx, int enable)
{
    ...
    vr_uvhm_set_vring_enable(vru_cl, vring_idx, enable);
    return 0;
}
