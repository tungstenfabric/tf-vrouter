...
class TestTcpFinResetBGP(unittest.TestCase):
    ...
    def establish_flow(self):
        ...
        tcp = TCP(flags='S', seq=1, sport=1136, dport=179)
        syn_pkt = ether_1 / ip_1 / tcp
        syn_pkt.show()
        self.vif3.send_packet(syn_pkt)

        tcp = TCP(flags='SA', sport=1136, dport=179, seq=1,
                  ack=syn_pkt.seq + 1)
        syn_ack_pkt = ether_2 / ip_2 / tcp
        syn_ack_pkt.show()
        self.vif4.send_packet(syn_ack_pkt)

        tcp = TCP(flags='A', sport=1136, dport=179, seq=syn_ack_pkt.ack,
                  ack=syn_ack_pkt.seq + 1)
        ack_pkt = ether_1 / ip_1 / tcp
        ack_pkt.show()
        self.vif3.send_packet(ack_pkt)

        tcp = TCP(flags='R', sport=1136, dport=179, seq=ack_pkt.ack)
        rst_pkt = ether_2 / ip_2 / tcp
        rst_pkt.show()

        tcp = TCP(flags='F', sport=1136, dport=179, seq=ack_pkt.ack)
        fin_pkt = ether_2 / ip_2 / tcp
        fin_pkt.show()
        return fin_pkt, rst_pkt


    def test_flow_on_tcp_finrst_for_bgp_port(self):

        fin_pkt, rst_pkt = self.establish_flow()

        # case where rst packet on port 179(BGP)  is tested
        check_flow_delete_cmd = ("flow --get ")
        self.vif3.send_packet(rst_pkt)
        ...
        #case where FIN packet on port 179(BGP) is tested
        check_flow_delete_cmd = ("flow --get ")
        self.vif3.send_packet(fin_pkt)
        check_flow_delete_cmd += str(self.f_flow.get_fr_index())
        flow_stats = ObjectBase.get_cli_output(check_flow_delete_cmd)
        self.assertEqual(0, ("EVICTED" in flow_stats))
