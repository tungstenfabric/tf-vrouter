...
test_fill_offload_entry_2_vms_l2(void **state)
{
    ...
    add_nh_to_bridge_table(vrf_id, dst_mac, entry.dst_nh->id, 1);
...
test_fill_offload_entry_mirroring_per_flow(void **state)
{
    ...
    create_offload_nexthop(mirror_nexthop_id, NH_ENCAP, NH_FLAG_VALID, mirror_vif_id, AF_BRIDGE);
    create_offload_mirror(mirror_id, mirror_nexthop_id);
...
test_fill_offload_entry_mirroring_per_interface(void **state)
{
    ...
    entry.src_vif = create_vif(entry.src_nh->interface_id,
        VR_MAX_MIRROR_INDICES);
    entry.dst_vif = create_vif(entry.dst_nh->interface_id, mirror_id);
    entry.mirror_vif = create_vif(mirror_vif_id, VR_MAX_MIRROR_INDICES);
...
test_fill_offload_entry_two_mirrors(void **state)
{
    ...
    create_offload_nexthop(flow_mirror_nexthop_id, NH_ENCAP, NH_FLAG_VALID,
        flow_mirror_vif_id, AF_BRIDGE);
    create_offload_mirror(flow_mirror_id, flow_mirror_nexthop_id);

    entry.src_vif = create_vif(entry.src_nh->interface_id,
        VR_MAX_MIRROR_INDICES);
    entry.dst_vif = create_vif(entry.dst_nh->interface_id,
        interface_mirror_id);
...
test_fill_offload_entry_mirroring_per_flow_is_broken(void **state)
{
    ...
    create_offload_mirror(interface_mirror_id, interface_mirror_nexthop_id);
