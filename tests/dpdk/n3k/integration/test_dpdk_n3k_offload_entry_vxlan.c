...
test_fill_offload_entry_2_vms_with_routing_vxlan(void **state)
{
    ...
    add_nh_to_bridge_table(vrf_id, dst_mac, l2_rcv_nexthop->id, 1);

    entry.dst_nh = create_offload_nexthop(dst_nh_id, NH_ENCAP, NH_FLAG_VALID, dst_vif_id, AF_INET);
    add_nh_to_route_table(vrf_id, dst_ip, entry.dst_nh->id, 0);
...
test_fill_offload_entry_with_vxlan_lookup(void **state)
{
    ...
    add_nh_to_bridge_table(vrf_id, dst_mac, entry.dst_nh->id, vni);
    ...
    create_offload_vxlan(vni, entry.dst_nh->id);
