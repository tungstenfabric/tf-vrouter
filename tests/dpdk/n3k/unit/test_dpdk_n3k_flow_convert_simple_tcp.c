...
test_same_cn_same_net_forward(void **state)
{
    ...
    struct rte_flow_action_vf *vf_action_conf =
        (struct rte_flow_action_vf *)flow_package.actions[ACTION_PORT_ID].conf;
...
main(void)
{
    ...
    return cmocka_run_group_tests_name(
        "vr_dpdk_n3k_flow_convert_simple_tcp", tests, NULL, NULL);
}
