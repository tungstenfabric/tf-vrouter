...
test_parse_packet_key_from_udp_on_vm_rx(void **state)
{
    ...
    assert_int_equal(key.ip.src.ipv4, RTE_IPV4(192, 168, 0, 1));
    assert_int_equal(key.ip.dst.ipv4, RTE_IPV4(192, 168, 0, 2));
...
test_parse_packet_metadata_from_udp_on_vm_rx(void **state)
{
    ...
    assert_memory_equal(
        ...
        test_udp_packets[0].eth_hdr.s_addr.addr_bytes,
        ETHER_ADDR_LEN);
    assert_memory_equal(
        metadata.inner_dst_mac,
        test_udp_packets[0].eth_hdr.d_addr.addr_bytes,
        ETHER_ADDR_LEN);
}
...
test_parse_packet_metadata_from_tcp_on_vm_rx(void **state)
{
    ...
    assert_memory_equal(
        metadata.inner_src_mac,
        test_tcp_packets[0].eth_hdr.s_addr.addr_bytes,
        ETHER_ADDR_LEN);
    assert_memory_equal(
        metadata.inner_dst_mac,
        test_tcp_packets[0].eth_hdr.d_addr.addr_bytes,
        ETHER_ADDR_LEN);
}
...
static void
test_parse_packet_ipv6_key_from_udp_on_vm_rx(void **state)
{
    struct vr_packet pkt = {
        .vp_if = &VIRTUAL_1_VIF,
        .vp_nh = &test_nexthops[VIRTUAL_1_VIF.vif_nh_id],
        .vp_head = (uint8_t *)(&test_udp_packets_ipv6[0]),
        .vp_data = 0,
    };
    struct vr_dpdk_n3k_packet_key key;
    struct vr_dpdk_n3k_packet_metadata metadata;
    ...
    assert_memory_equal(key.ip.src.ipv6,
                        test_udp_packets_ipv6[0].ipv6_hdr.src_addr,
                        VR_IP6_ADDRESS_LEN);
    assert_memory_equal(key.ip.dst.ipv6,
                        test_udp_packets_ipv6[0].ipv6_hdr.dst_addr,
                        VR_IP6_ADDRESS_LEN);
    assert_int_equal(key.proto, VR_IP_PROTO_UDP);
    assert_int_equal(key.src_port, rte_cpu_to_be_16(5555));
    assert_int_equal(key.dst_port, rte_cpu_to_be_16(6666));
...
static void
test_parse_packet_ipv6_metadata_from_udp_on_vm_rx(void **state)
{
    struct vr_packet pkt = {
        .vp_if = &VIRTUAL_1_VIF,
        .vp_nh = &test_nexthops[VIRTUAL_1_VIF.vif_nh_id],
    ...
    assert_memory_equal(
        metadata.inner_src_mac,
        test_udp_packets_ipv6[0].eth_hdr.s_addr.addr_bytes,
        ETHER_ADDR_LEN);
    assert_memory_equal(
        metadata.inner_dst_mac,
        test_udp_packets_ipv6[0].eth_hdr.d_addr.addr_bytes,
        ETHER_ADDR_LEN);
}

static void
test_parse_packet_ipv6_key_from_tcp_on_vm_rx(void **state)
{
    struct vr_packet pkt = {
        .vp_if = &VIRTUAL_1_VIF,
        .vp_nh = &test_nexthops[VIRTUAL_1_VIF.vif_nh_id],
        .vp_head = (uint8_t *)(&test_tcp_packets_ipv6[0]),
        .vp_data = 0,
    };
    struct vr_dpdk_n3k_packet_key key;
    struct vr_dpdk_n3k_packet_metadata metadata;
    ...
    assert_int_equal(key.ip.type, VR_N3K_IP_TYPE_IPV6);
    assert_memory_equal(key.ip.src.ipv6,
                        test_tcp_packets_ipv6[0].ipv6_hdr.src_addr,
                        VR_IP6_ADDRESS_LEN);
    assert_memory_equal(key.ip.dst.ipv6,
                        test_tcp_packets_ipv6[0].ipv6_hdr.dst_addr,
                        VR_IP6_ADDRESS_LEN);
    assert_int_equal(key.proto, VR_IP_PROTO_TCP);
    assert_int_equal(key.src_port, rte_cpu_to_be_16(5555));
    assert_int_equal(key.dst_port, rte_cpu_to_be_16(6666));
...
static void
test_parse_packet_ipv6_metadata_from_tcp_on_vm_rx(void **state)
{
    struct vr_packet pkt = {
        .vp_if = &VIRTUAL_1_VIF,
        .vp_nh = &test_nexthops[VIRTUAL_1_VIF.vif_nh_id],
    ...
    assert_memory_equal(
        metadata.inner_src_mac,
        test_tcp_packets_ipv6[0].eth_hdr.s_addr.addr_bytes,
        ETHER_ADDR_LEN);
    assert_memory_equal(
        metadata.inner_dst_mac,
        test_tcp_packets_ipv6[0].eth_hdr.d_addr.addr_bytes,
        ETHER_ADDR_LEN);
}
...
main(void)
{
    const struct CMUnitTest tests[] = {
        ...
        /* IPv6 */
        cmocka_unit_test(test_parse_packet_ipv6_key_from_udp_on_vm_rx),
        cmocka_unit_test(test_parse_packet_ipv6_metadata_from_udp_on_vm_rx),
        cmocka_unit_test(test_parse_packet_ipv6_key_from_tcp_on_vm_rx),
        cmocka_unit_test(test_parse_packet_ipv6_metadata_from_tcp_on_vm_rx),
    };
