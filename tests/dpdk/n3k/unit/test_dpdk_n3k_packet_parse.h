...
struct packet_udp_ipv6 {
    struct rte_ether_hdr eth_hdr;
    struct rte_ipv6_hdr ipv6_hdr;
    struct rte_udp_hdr udp_hdr;
} __attribute__((packed));

static struct packet_udp_ipv6 test_udp_packets_ipv6[] = {
    // Ethernet(type=IPv6)/IPv6(proto=UDP)/UDP()
    {
        .eth_hdr = {
            .s_addr = {
                .addr_bytes = { 0x54, 0x52, 0x00, 0x00, 0x00, 0x01 },
        ...
        .ipv6_hdr = {
            .vtc_flow = RTE_BE32(0xfful << RTE_IPV6_HDR_TC_SHIFT),
            .proto = IPPROTO_UDP,
...
struct packet_tcp_ipv6 {
    struct rte_ether_hdr eth_hdr;
    struct rte_ipv6_hdr ipv6_hdr;
    struct rte_tcp_hdr tcp_hdr;
} __attribute__((packed));

static struct packet_tcp_ipv6 test_tcp_packets_ipv6[] = {
    // Ethernet(type=IPv6)/IPv6(proto=TCP)/TCP()
    {
        .eth_hdr = {
            .s_addr = {
                .addr_bytes = { 0x54, 0x52, 0x00, 0x00, 0x00, 0x01 },
