...
test_mpls_o_udp_decap_tcp(void **state)
{
    ...
    struct vr_n3k_offload_nexthop src_nh = {
        .type = NH_TUNNEL,
        .src_mac = { 0xde, 0xad, 0xbe, 0xef, 0x00, 0x00 },
        .dst_mac = { 0xde, 0xad, 0xc0, 0xde, 0x00, 0x00 },
    ...
    struct vr_n3k_offload_entry offload_entry = {
        ...
        .flow = &flow,
        .tunnel_type = VR_N3K_OFFLOAD_TUNNEL_MPLS,
...
test_mpls_o_udp_encap_tcp(void **state)
{
    ...
    const uint8_t outer_src_mac[VR_ETHER_ALEN] = { 0x00, 0xde, 0xad, 0xc0, 0xde, 0x00 };
    const uint8_t outer_dst_mac[VR_ETHER_ALEN] = { 0x00, 0xde, 0xad, 0xbe, 0xef, 0x00 };
    ...
    memcpy(dst_nh.src_mac, outer_src_mac, VR_ETHER_ALEN);
    memcpy(dst_nh.dst_mac, outer_dst_mac, VR_ETHER_ALEN);
    ...
    assert_memory_equal(ether_hdr->s_addr.addr_bytes, outer_src_mac, VR_ETHER_ALEN);
    assert_memory_equal(ether_hdr->d_addr.addr_bytes, outer_dst_mac, VR_ETHER_ALEN);
    assert_int_equal(ether_hdr->ether_type, RTE_BE16(VR_ETH_PROTO_IP));
...
test_mpls_o_udp_decap_udp_l3(void **state)
{
    ...
    memcpy(dst_nh.dst_mac, dst_mac, VR_ETHER_ALEN);
