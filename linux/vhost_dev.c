    ...
    for (i = 0; i < VHOST_MAX_INTERFACES; i++) {
        ...
        if (vp && !strncmp(dev->name, vp->vp_phys_name, VR_INTERFACE_NAME_LEN)) {
            ...
            if (!vifp || pkt->vp_send_thru_vrouter) {
                ...
                skb->dev = vp->vp_dev;   // emulate coming from vhost0

                // We received packets from the kernel here; semantically from vhost0; count them
                (void)__sync_fetch_and_add(&vp->vp_dev->stats.tx_packets, 1);
                (void)__sync_fetch_and_add(&vp->vp_dev->stats.tx_bytes, skb->len);
                // vhost_rx counts the custom stats
...
static void set_phys_start_xmit(struct vhost_priv *vp, struct net_device *pdev)
{
    ...
    WRITE_ONCE(custom_phys_start_xmit_ptr, &phys_start_xmit);

    // Overwrite phys ndo_start_xmit() with a custom version that sends the packet
    // thru. vrouter tx pipeline
    WRITE_ONCE(vp->phys_dev_ops.ndo_start_xmit, custom_phys_start_xmit_ptr);

    // Save the original netdev_ops
    WRITE_ONCE(vp->orig_phys_dev_ops, (struct net_device_ops *)pdev->netdev_ops);
...
void vhost_phys_netdev_ops_exit(void)
{
    ...
    for (i = 0; i < VHOST_MAX_INTERFACES; i++) {
        vp = vhost_priv_db[i];
        if(vp && vp->vp_phys_dev && vp->vp_phys_dev->netdev_ops && vp->orig_phys_dev_ops) {
