...
define dump_dpdk_global
    ...
    printf "nb_fwd_lcores:%hd\n", vr_dpdk.nb_fwd_lcores
    printf "nb_io_lcores:%hd\n", vr_dpdk.nb_io_lcores
    printf "nb_free_mempools:%hd\n", vr_dpdk.nb_free_mempools
...
define dump_rte_ring
    ...
    printf "Producer:\n    head:%u\n", $arg0.prod.head
    printf "    tail:%u\n    is_single_prod:%u\n", $arg0.prod.tail, $arg0.prod.single
    printf "Consumer:\n    head:%u\n", $arg0.cons.head
    printf "    tail:%u\n    is_single_cons:%u\n", $arg0.cons.tail, $arg0.cons.single
    set $used_val = 0
...
define dump_dpdk_lcore_all
    ...
    while($lcore_iter < $vr_max_cpus)
        if(vr_dpdk.lcores[$lcore_iter])
            printf "lcore ID:%d\n", $lcore_iter
            dump_dpdk_lcore vr_dpdk.lcores[$lcore_iter] $lcore_iter
        end
        set $lcore_iter += 1
    end
end
...
define print_ethdev_slave_ids
    ...
    while($slave_iter<$arg0)
        printf "    rte_eth_device_id:%d", $arg1[$slave_iter]
        printf "    driver:%s\n", rte_eth_devices[$arg1[$slave_iter]].device.driver.name
        set $slave_iter += 1
    end
end
...
#order of eth_stats: ipackets, opackets, ibytes, obytes, imissed, ierrors, oerrors
    if($_streq($arg1.name, "net_bonding"))
		...
		set $slave_port_id = $internal.slaves[$slavecounter].port_id
                set $cur_slave = rte_eth_devices[$slave_port_id]
    ...
    if($stats_set == 1)
        ...
        set $driver_name = $arg1.name
        if($_streq($arg1.name, "net_bonding"))
            set $driver_name = $slave_name
        end
        printf "Stats: not supported for %s driver type!\n", $driver_name
    end
end
...
define eth_igb_get_stats
    ...
    set $igb_stats = ($igb_adapter.stats)
    if($igb_stats)
...
define eth_ixgbe_get_stats
    ...
    set $ixgbe_stats = ($ixgbe_adapter.stats)
    if($ixgbe_stats)
        set $total_qprc = 0
        set $total_qbrc = 0
        set $total_qprdc = 0
...
#arg0:lcore_index
define dump_dpdk_lcore_index
    if(vr_dpdk.lcores[$arg0])
        dump_dpdk_lcore vr_dpdk.lcores[$arg0] $arg0
    else
        printf "Null pointer\n"
    end
end

#arg0:vr_dpdk_lcore pointer, arg1:lcore_id
define dump_dpdk_lcore
    print_lcore_type $arg1
...
document dump_dpdk_lcore_index
Syntax:dump_dpdk_lcore_index lcore_index
No. of arguments:1
...
define print_dpdk_q_slist
    ...
    if($arg1 == 0)
        printf "rx_rings_serviced: "
    else
        printf "tx_rings_serviced: "
    end
    if(!($cur_ele))
        printf "-"
    end
    while($cur_ele)
        printf "vif 0/%u, ", $cur_ele.q_vif.vif_idx
