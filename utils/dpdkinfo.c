...
parse_long_opts(int opt_index, char *opt_arg)
{
    ...
    case XSTATS_OPT_INDEX:
        ...
        if (!strcmp(opt_arg, "all") || !strcmp(opt_arg, "") ||
            !strcmp(opt_arg, "0") || ((atoi(opt_arg) > 0) &&
                           (atoi(opt_arg) <= VR_INTERFACE_BOND_MAX_SLAVES))){
            vr_info_inbuf = opt_arg;
        } else {
            Usage();
        }
    ...
    case BUFFSZ_OPT_INDEX:
        /* Setting buffsz limit to 1000Mb */
        if ((unsigned)strtol(opt_arg, NULL, 0) >= 0x3E800000){
            Usage();
        }
