...
DPDKAgent = False
OPERATING_SYSTEM_CENTOS = False
GDB_Flag = True
Docker_Copy_Stripped = False


#Change before running
debugger_location = '/root/CEM-1289/tf_debugger/'
...
URL = ['http://10.84.5.100/cs-shared/github-build/',
'/redhat70/newton/sandbox/build/debug/vnsw/agent/contrail/contrail-vrouter-agent',
'/redhat70/newton/sandbox/build/debug/control-node/contrail-control']
...
def WhereIsMyDocker(Hostname, BuildVersion, IDVersion):
  ...
  if ('centos' in build_host_name ):
    if(OPERATING_SYSTEM_CENTOS):
      RedHatOs = False
  else:
    if('redhat' in build_host_name ):
      if(not OPERATING_SYSTEM_CENTOS):
        RedHatOs = True
      else:
        print("The operating system you are on is "+p2+" and this is a gcore which was not generated on the same operating system, please run the script on appropriate machine to get the right results \n")
        sys.exit()
    else:
        print("The operating system you are on is "+p2+" and this is a gcore which was not generated on the same operating system, please run the script on appropriate machine to get the right results \n")
        sys.exit()

  if (BuildVersion == '1910'): # Format for version 1910 is a little different, this is to handle that situation
    if (RedHatOs):
      if(ControlNodeAgent):
        ...
        print("This isnt a Vrouter agent, Dpdk or control node. Cannot process it. Please provide the right file")
        sys.exit()
    else:
      if(ControlNodeAgent):
    ...
    if (RedHatOs):
      if(ControlNodeAgent):
        ...
        docker_path = base_path+"/contrail-debug:"+build_version_number+"."+build_id_version+'-rhel'
        docker_path_stripped = base_path+"/contrail-vrouter-agent-dpdk:"+build_version_number+"."+build_id_version+'-rhel'
      elif(VRouterAgent): #REVIEW ERROR CONDITION
        docker_path = base_path+"/contrail-debug:"+build_version_number+"."+build_id_version+'-rhel'
        docker_path_stripped = base_path+"/contrail-vrouter-agent:"+build_version_number+"."+build_id_version+'-rhel'
      else:
        print("This isnt a Vrouter agent, Dpdk or control node. Cannot process it. Please provide the right file")
...
def DockerCopy(build_version_number,build_id_version,PathOfTheDocker, NameOfTheFile, BinaryLocation):
  ...
  creating_new_directory_path = subprocess.run(['mkdir',debugger_location+build_id_version+"_"+build_version_number],stdout=subprocess.PIPE)
  if(not Docker_Copy_Stripped):
        copier = subprocess.run(['docker','cp',docker_names_list[0] + debug_location ,debugger_location+build_id_version+"_"+build_version_number+"/"+file_name])
  else:
    if (ControlNodeAgent):
        copier = subprocess.run(['docker','cp',docker_names_list[0] + debug_location+"contrail-control", debugger_location+build_id_version+"_"+build_version_number+"/"+file_name])
    elif(DPDKAgent or VRouterAgent):
        copier = subprocess.run(['docker','cp',docker_names_list[0] + debug_location+"contrail-vrouter-agent" ,debugger_location+build_id_version+"_"+build_version_number+"/"+file_name])
    else:
        print("This isnt a Vrouter agent, Dpdk or control node. Cannot process it. Please provide the right file")
        sys.exit()
...
def binary_get(BuildVersion, IDVersion,PathOfTheDocker, PathOfTheDockerStripped ):
  ...
  if(build_version_number in containerized_versions):
    ...
    with open( debugger_location+build_id_version+"_"+build_version_number+'/contrail-vrouter-agent.debug', 'wb') as local_file:
        local_file.write(file_object.content)
    file_object = requests.get(file_url_stripped)
    with open(debugger_location+build_id_version+"_"+build_version_number+'/contrail-vrouter-agent-stripped', 'wb') as local_file:
        local_file.write(file_object.content)
...
def Running_GDB_Custom(ID, VERSION, core_file_name, GdbScript):
  ...
  move = subprocess.run(['cp', core_file_name, debugger_location+build_id_version+"_"+build_version_number+"/" ])
  move_GDB_script = subprocess.run(['cp', "Buffered_GDB_output.txt", debugger_location+build_id_version+"_"+build_version_number+"/" ], stdout=subprocess.PIPE)
  change_dir = subprocess.run(['cd',build_id_version+"_"+build_version_number], shell=True)
  pwd = subprocess.run(['pwd'],cwd= debugger_location+build_id_version+"_"+build_version_number, stdout=subprocess.PIPE)
...
def Running_GDB(ID, VERSION, core_file_name):
  ...
  fin.close()
  fout.close()
  move = subprocess.run(['cp', core_file_name, debugger_location+build_id_version+"_"+build_version_number+"/" ], stdout=subprocess.PIPE)
  move_GDB_script = subprocess.run(['cp', "Buffered_GDB_output.txt", debugger_location+build_id_version+"_"+build_version_number+"/" ], stdout=subprocess.PIPE)
  change_dir = subprocess.run(['cd',build_id_version+"_"+build_version_number], shell=True)
  pwd = subprocess.run(['pwd'],cwd= debugger_location+build_id_version+"_"+build_version_number, stdout=subprocess.PIPE)
  running_GDB = subprocess.run(['gdb' ,'-x', 'Buffered_GDB_output.txt'], cwd= debugger_location+build_id_version+"_"+build_version_number, stdout=subprocess.PIPE)
...
def main():
    ...
    if(n ==1 and (COMMAND_ARGUMENT == '--help' or COMMAND_ARGUMENT == '-h')):
      ...
      if((("vrouter-agent") in Agent_name)):
        ...
        binary_get(VERSION_BY_USER, ID_BY_USER, DOCKER_PATH_UNSTRIPPED, DOCKER_PATH_STRIPPED)
        print("\n\nRunning GDB...\n\n")
        CALLING_GDB = Running_GDB_Custom(ID_BY_USER, VERSION_BY_USER, COMMAND_ARGUMENT, GDBSCRIPT)
        print()
        Summary(VERSION_BY_USER,ID_BY_USER, HOST, TIME, AGENT)
