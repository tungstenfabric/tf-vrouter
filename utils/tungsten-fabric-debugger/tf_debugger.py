...
#base_path_Stripped = "svl-artifactory.juniper.net/contrail-nightly/contrail-vrouter-agent:"

your_location = '/root/CEM-1289/tf_debugger/'

Production_Binary = your_location+'/contrail-vrouter-agent'
  ...
  if ('centos' in build_host_name ):
    ...
    if('redhat' in build_host_name ):
      if(not OPERATING_SYATEM_CENTOS):
        ...
        print("the gcore is and the operating system doesnot match,please run on aproppriate system \n")
        sys.exit()
...
def binary_get(BiResult, BuildVersion, IDVersion,PathOfTheDocker, PathOfTheDockerStripped ):   
  p7 = BiResult
  build_version_number = BuildVersion
  build_id_version = IDVersion
  ...
  if(p7 == "not stripped"):
   if(build_version_number in containerized_versions):
      ...
      if (VRouterAgent or DPDKAgent):
          ...
          #print("\n URL : "+ file_url_unstripped+ "\n")
          #print("\n URL : "+ file_url_stripped+ "\n")
      else:
          usable_version_number = GenerateURl(version)
...
def Running_GDB_Custom(ID, VERSION, core_file_name, GdbScript):
  ...
  fin = open(copier)
  fout = open("subprocess_gdb_output.txt", "wt")
  ...
  move_GDB_script = subprocess.run(['cp', "subprocess_gdb_output.txt", your_location+build_id_version+"_"+build_version_number+"/" ], stdout=subprocess.PIPE)
  change_dir = subprocess.run(['cd',build_id_version+"_"+build_version_number], shell=True)
  #print(str(change_dir.stdout))
  pwd = subprocess.run(['pwd'],cwd= your_location+build_id_version+"_"+build_version_number, stdout=subprocess.PIPE)
  #running_GDB = subprocess.run(['gdb',"/root/CEM-1289/tf_debugger/"+build_id_version+"_"+build_version_number+ "/contrail-vrouter-agent", core_file_name, '-x', 'subprocess_gdb_output.txt'])
  running_GDB = subprocess.run(['gdb' ,'-x', 'subprocess_gdb_output.txt'], cwd= your_location+build_id_version+"_"+build_version_number)
  pwd = subprocess.run(['pwd'])

  #running_GDB = subprocess.run(['gdb',your_location+build_id_version+"_"+build_version_number+ "/contrail-vrouter-agent" , core_file_name, '-x', 'subprocess_gdb_output.txt' ])
...
def Running_GDB(ID, VERSION, core_file_name):
  ...
  fin = open("tester_gdb_commands.txt")
  fout = open("subprocess_gdb_output.txt", "wt")
...
def Vrouter(COMMAND_ARGUMENT):
  ...
  #print("DOCKER PATH OF THE CONTAINER (Unstripped Binary): "+ DOCKER_PATH_UNSTRIPPED+"\n\n")
  #print("DOCKER PATH OF THE CONTAINER (Stripped Binary): "+ DOCKER_PATH_STRIPPED+"\n\n")
  binary_get(BINARY_RESULT, VERSION, ID, DOCKER_PATH_UNSTRIPPED, DOCKER_PATH_STRIPPED)
  print("\n\nRunning GDB...\n\n")
...
def DPDK(COMMAND_ARGUMENT):
  ...
  print("fetching info....................")
  FILE_INFO = core_info(COMMAND_ARGUMENT) 
  VERSION, HOST, ID = Build_Version_ID_HOST_DPDK(FILE_INFO)# getting version and Usable version
  #print("VERSION:   " + VERSION)
  #print("ID   :" + ID)
  #print("HOST   :" + HOST)
  TIME = Build_Time(FILE_INFO) # getting tim
...
def main():
    ...
    if(n ==1 and (COMMAND_ARGUMENT == '--help' or COMMAND_ARGUMENT == '-h')):
      ...
      #print("Version:  "+ VERSION_BY_USER)
      #print("ID:   "+ ID_BY_USER)
      TIME = Build_Time(FILE_INFO)
      #print("Build Time:   "+ TIME)
      HOST = Build_Host(FILE_INFO)
      #print("Build HostName:   "+ HOST)
      BINARY_RESULT = binary_check()
      DOCKER_PATH_UNSTRIPPED, DOCKER_PATH_STRIPPED = WhereIsMyDocker(HOST, VERSION, ID ) 
      binary_get(BINARY_RESULT, VERSION, ID, DOCKER_PATH_UNSTRIPPED, DOCKER_PATH_STRIPPED)
      print("\n\nRunning GDB...\n\n")
