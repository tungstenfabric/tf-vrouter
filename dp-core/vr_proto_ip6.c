...
bool
vr_ipv6_nd_input(struct vr_packet *pkt,
                 struct vr_forwarding_md *fmd)
{
    ...
    icmph = (struct vr_icmp *) ((unsigned char *) ip6 + sizeof(struct vr_ip6));
    data = pkt_data(pkt);
    ...
    switch (icmph->icmp_type) {
        ...
        case VR_ICMP6_TYPE_NEIGH_AD:
            handled = vr_neighbor_reply(icmph, pkt, fmd);
            break;

        case VR_ICMP6_TYPE_ROUTER_SOL:
        case VR_ICMP6_TYPE_ROUTER_AD:
            return vif_xconnect(pkt->vp_if, pkt, fmd);
