        ...
        int8_t *prefix, int level)
{
    ...
    if (ENTRY_IS_BUCKET(orig_ent)) {
        ...
        if (!dumper->dump_been_to_marker) {
            ...
            /*
             * The following condition ensures that return 0 is not called
             * without creating a dump response if the entry is default nexthop
             * entry. Without this condition, the rt command does not generate
             * a response for the 0/0 or 0/8 route.
             */
...
mtrie_stats_get(vr_vrf_stats_req *req, vr_vrf_stats_req *response)
{
    ...
    for (i = 0; i < vr_num_cpus; i++) {
        ...
        if (stats) {
            ...
            response->vsr_mpls_receives += stats->vrf_mpls_receives;
        }
    }
...
mtrie_stats_empty(vr_vrf_stats_req *r)
{
    if (r->vsr_discards || r->vsr_resolves || r->vsr_receives ||
            ...
            r->vsr_udp_mpls_over_mpls_tunnels || r->vsr_mpls_receives)
        return false;
